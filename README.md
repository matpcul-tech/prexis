...
- **Questions that never repeat**: dozens of curated questions are parameterized generators, the same trick Brilliant uses. A question like "What is 15% of 80?" carries named number ranges and a formula, and it re-rolls with fresh numbers every single time it appears: in the lesson, on a replay, in practice, in review. One authored question becomes hundreds. Every generator is machine-checked against its original: the formula must reproduce the authored answer at the authored numbers.
