          ...
          lessons: [
            {
              ...
              steps: [
                ...
                { type: "code", prompt: "Write double so it returns twice its input.", starter: "function double(n) {\n  \n}", tests: [ { call: "double(2)", expect: 4 }, { call: "double(0)", expect: 0 }, { call: "double(-3)", expect: -6 } ], solution: "function double(n) {\n  return n * 2;\n}", explain: "Multiply and return. No console.log needed, tests read the return value.", gen: { variants: [{ prompt: "Write halve so it returns half its input.", starter: "function halve(n) {\n  \n}", tests: [{ call: "halve(8)", expect: 4 }, { call: "halve(0)", expect: 0 }, { call: "halve(-6)", expect: -3 }], solution: "function halve(n) {\n  return n / 2;\n}", explain: "Return the result. Printing it is not the same as returning it." }, { prompt: "Write triple so it returns three times its input.", starter: "function triple(n) {\n  \n}", tests: [{ call: "triple(2)", expect: 6 }, { call: "triple(0)", expect: 0 }, { call: "triple(-2)", expect: -6 }], solution: "function triple(n) {\n  return n * 3;\n}", explain: "Return the result. Printing it is not the same as returning it." }] } },
                { type: "order", prompt: "Put these in the order JavaScript actually does them.", items: ["Read the function body", "Bind the parameter n to 4", "Evaluate n * 2", "Hand 8 back to the caller"], explain: "A call first binds arguments, then runs the body, then returns.", codeItems: false, gen: { variants: [{ prompt: "Put these in the order JavaScript runs a call to greet(\"Ada\").", items: ["Bind the parameter name to \"Ada\"", "Run the function body", "Build the greeting string", "Hand the string back to the caller"], explain: "Arguments bind first, the body runs, and the return value travels back to the call site.", codeItems: false }] } },
            ...
            },
            {
              ...
              steps: [
                ...
                { type: "code", prompt: "Return only the even numbers from the list.", starter: "function evens(list) {\n  \n}", tests: [ { call: "evens([1,2,3,4])", expect: [2, 4] }, { call: "evens([7])", expect: [] }, { call: "evens([0, 8])", expect: [0, 8] } ], solution: "function evens(list) {\n  return list.filter(n => n % 2 === 0);\n}", explain: "filter keeps items whose callback returns true. 0 is even.", gen: { variants: [{ prompt: "Return only the words longer than three letters.", starter: "function longWords(words) {\n  \n}", tests: [{ call: "longWords([\"hi\", \"door\", \"a\", \"gates\"])", expect: ["door", "gates"] }, { call: "longWords([\"at\"])", expect: [] }], solution: "function longWords(words) {\n  return words.filter((w) => w.length > 3);\n}", explain: "filter keeps the items the test says yes to and builds a new list from them." }, { prompt: "Return only the numbers greater than zero from the list.", starter: "function positives(list) {\n  \n}", tests: [{ call: "positives([-2, 5, 0, 9])", expect: [5, 9] }, { call: "positives([-1, -3])", expect: [] }, { call: "positives([1])", expect: [1] }], solution: "function positives(list) {\n  return list.filter((n) => n > 0);\n}", explain: "filter keeps the items the test says yes to and builds a new list from them." }] } },
                { type: "order", prompt: "Order these lines so the last expression is the doubled list.", items: ["const src = [1, 2, 3];", "const out = src.map(n => n * 2);", "out"], explain: "Create the source, derive a new array, then read it.", codeItems: true, gen: { variants: [{ prompt: "Order these lines so the last expression is the uppercased word.", items: ["const w = \"hi\";", "const up = w.toUpperCase();", "up"], explain: "Define, transform, then read the result. Each line needs the one before it.", codeItems: true }] } },
            ...
            },
            {
              ...
              steps: [
                ...
                { type: "code", prompt: "Write safeName: return user.name if user exists, otherwise \"guest\".", starter: "function safeName(user) {\n  \n}", tests: [ { call: "safeName({name:\"Ada\"})", expect: "Ada" }, { call: "safeName(undefined)", expect: "guest" }, { call: "safeName(null)", expect: "guest" } ], solution: "function safeName(user) {\n  return (user && user.name) || \"guest\";\n}", explain: "Guard the object before you touch a property. null and undefined are both unsafe.", gen: { variants: [{ prompt: "Write safeAge: return user.age if user exists, otherwise 0.", starter: "function safeAge(user) {\n  \n}", tests: [{ call: "safeAge({age:30})", expect: 30 }, { call: "safeAge(undefined)", expect: 0 }, { call: "safeAge(null)", expect: 0 }], solution: "function safeAge(user) {\n  return (user && user.age) || 0;\n}", explain: "Guard before you reach: only read .age once you know user is really there." }, { prompt: "Write safeCity: return user.city if user exists, otherwise \"unknown\".", starter: "function safeCity(user) {\n  \n}", tests: [{ call: "safeCity({city:\"Oslo\"})", expect: "Oslo" }, { call: "safeCity(undefined)", expect: "unknown" }, { call: "safeCity(null)", expect: "unknown" }], solution: "function safeCity(user) {\n  return (user && user.city) || \"unknown\";\n}", explain: "Guard before you reach: only read .city once you know user is really there." }] } },
                { type: "order", prompt: "A debugging order that usually works.", items: ["Reproduce it once", "Read the exact error", "Find the first bad value", "Change one thing"], explain: "If you change three things at once you will not know which fix worked.", codeItems: false, gen: { variants: [{ prompt: "A calm bug hunt, in order.", items: ["Trigger the bug on purpose", "Read the message and the line number", "Inspect the value that arrives there", "Fix one thing and rerun"], explain: "Reproduce, read, inspect, change one thing. Shotgun edits destroy the information each run gives you.", codeItems: false }] } },
          ...
          lessons: [
            {
              ...
              steps: [
                ...
                { type: "code", prompt: "Write plusLater so it returns a Promise that fulfills with a + b.", starter: "function plusLater(a, b) {\n  \n}", tests: [ { call: "plusLater(2, 3).then(x => x)", expect: 5 }, { call: "plusLater(0, 0).then(x => x)", expect: 0 } ], solution: "function plusLater(a, b) {\n  return Promise.resolve(a + b);\n}", explain: "Return a Promise, not the raw number. Promise.resolve wraps a ready value.", gen: { variants: [{ prompt: "Write minusLater so it returns a Promise that fulfills with a minus b.", starter: "function minusLater(a, b) {\n  \n}", tests: [{ call: "minusLater(5, 2).then(x => x)", expect: 3 }, { call: "minusLater(2, 2).then(x => x)", expect: 0 }], solution: "function minusLater(a, b) {\n  return Promise.resolve(a - b);\n}", explain: "Promise.resolve wraps a plain value in an already fulfilled promise." }, { prompt: "Write timesLater so it returns a Promise that fulfills with a times b.", starter: "function timesLater(a, b) {\n  \n}", tests: [{ call: "timesLater(2, 3).then(x => x)", expect: 6 }, { call: "timesLater(5, 0).then(x => x)", expect: 0 }], solution: "function timesLater(a, b) {\n  return Promise.resolve(a * b);\n}", explain: "Promise.resolve wraps a plain value in an already fulfilled promise." }] } },
                { type: "order", prompt: "Order the life of a successful fetch.", items: ["Call fetch", "Await the Response", "Read the body", "Use the data"], explain: "You cannot read the body before the response exists.", codeItems: false, gen: { variants: [{ prompt: "Order the life of loading and showing a user profile.", items: ["Request the profile from the server", "Await the response", "Parse the JSON body", "Render the profile on screen"], explain: "Network, then parse, then use. Each await marks a wait without freezing the page.", codeItems: false }] } },
            ...
            },
            {
              ...
              steps: [
                ...
                { type: "code", prompt: "Write fullName so it joins first and last with a space.", starter: "function fullName(p) {\n  \n}", tests: [ { call: "fullName({first:\"Ada\", last:\"Lovelace\"})", expect: "Ada Lovelace" }, { call: "fullName({first:\"Alan\", last:\"Turing\"})", expect: "Alan Turing" } ], solution: "function fullName(p) {\n  return p.first + \" \" + p.last;\n}", explain: "Read two drawers, concatenate with a space between.", gen: { variants: [{ prompt: "Write tagLine so it joins role and name with a colon and a space.", starter: "function tagLine(p) {\n  \n}", tests: [{ call: "tagLine({role:\"Captain\", name:\"Grace\"})", expect: "Captain: Grace" }, { call: "tagLine({role:\"Dr\", name:\"Alan\"})", expect: "Dr: Alan" }], solution: "function tagLine(p) {\n  return p.role + \": \" + p.name;\n}", explain: "Read both drawers, glue them with the separator you want." }, { prompt: "Write signature so it joins name and title with a comma and a space.", starter: "function signature(p) {\n  \n}", tests: [{ call: "signature({name:\"Ada\", title:\"Countess\"})", expect: "Ada, Countess" }, { call: "signature({name:\"Alan\", title:\"Dr\"})", expect: "Alan, Dr" }], solution: "function signature(p) {\n  return p.name + \", \" + p.title;\n}", explain: "Read both drawers, glue them with the separator you want." }] } },
                { type: "order", prompt: "Order what JavaScript does to evaluate user.pet.name.", items: ["Find the user object", "Open its pet drawer", "Open name inside that", "Hand back the value"], explain: "Chained dots resolve left to right, which is why an undefined middle link throws.", codeItems: false, gen: { variants: [{ prompt: "Order what JavaScript does to evaluate order.customer.email.", items: ["Find the order object", "Open its customer drawer", "Open email inside that", "Hand back the value"], explain: "Chained dots resolve left to right, one drawer at a time. An undefined middle link is where it throws.", codeItems: false }] } },
            ...
            },
            {
              ...
              steps: [
                ...
                { type: "code", prompt: "Write longest so it returns the longest word in the list.", starter: "function longest(list) {\n  \n}", tests: [ { call: "longest([\"hi\",\"hello\",\"hey\"])", expect: "hello" }, { call: "longest([\"a\"])", expect: "a" }, { call: "longest([\"aa\",\"bb\"])", expect: "aa" } ], solution: "function longest(list) {\n  return list.reduce((best, w) => (w.length > best.length ? w : best), list[0] || \"\");\n}", explain: "Keep the best-so-far as you walk the list. Ties keep the earlier word because the comparison is strictly greater.", gen: { variants: [{ prompt: "Write firstAlphabetical so it returns the word that comes first alphabetically.", starter: "function firstAlphabetical(words) {\n  \n}", tests: [{ call: "firstAlphabetical([\"pear\", \"apple\", \"plum\"])", expect: "apple" }, { call: "firstAlphabetical([\"b\", \"a\"])", expect: "a" }], solution: "function firstAlphabetical(words) {\n  return words.reduce((best, w) => (w < best ? w : best));\n}", explain: "Carry the current winner through the list, swapping whenever an earlier word appears." }, { prompt: "Write shortest so it returns the shortest word in the list.", starter: "function shortest(words) {\n  \n}", tests: [{ call: "shortest([\"banana\", \"fig\", \"apple\"])", expect: "fig" }, { call: "shortest([\"aa\", \"b\"])", expect: "b" }], solution: "function shortest(words) {\n  return words.reduce((best, w) => (w.length < best.length ? w : best));\n}", explain: "Carry the current winner through the list, swapping whenever a shorter word appears." }] } },
                { type: "order", prompt: "Arrange the lines into a working three-step pipeline.", items: ["const names = [\"ada\", \"alan\"];", "const caps = names.map(n => n.toUpperCase());", "console.log(caps.join(\", \"));"], explain: "Source data, then the transformation, then the output.", codeItems: true, gen: { variants: [{ prompt: "Arrange the lines into a working three-step pipeline.", items: ["const nums = [3, 1, 2];", "const sorted = [...nums].sort();", "console.log(sorted.join(\" then \"));"], explain: "Source, transform, output. The spread copy keeps nums itself unsorted.", codeItems: true }] } },
          ...
          lessons: [
            {
              ...
              steps: [
                ...
                { type: "code", prompt: "Write makeAdder so makeAdder(x) returns a function that adds x to its input.", starter: "function makeAdder(x) {\n  \n}", tests: [{"call": "makeAdder(3)(4)", "expect": 7}, {"call": "makeAdder(0)(5)", "expect": 5}, {"call": "makeAdder(-2)(2)", "expect": 0}], solution: "function makeAdder(x) {\n  return (y) => x + y;\n}", explain: "The returned arrow closes over x. Every adder you make remembers its own x.", gen: { variants: [{ prompt: "Write makeSubtractor so makeSubtractor(x) returns a function that subtracts x from its input.", starter: "function makeSubtractor(x) {\n  \n}", tests: [{ call: "makeSubtractor(3)(10)", expect: 7 }, { call: "makeSubtractor(0)(5)", expect: 5 }, { call: "makeSubtractor(-2)(2)", expect: 4 }], solution: "function makeSubtractor(x) {\n  return (y) => y - x;\n}", explain: "The returned function closes over x, so every call remembers what to subtract." }, { prompt: "Write makeMultiplier so makeMultiplier(x) returns a function that multiplies its input by x.", starter: "function makeMultiplier(x) {\n  \n}", tests: [{ call: "makeMultiplier(3)(4)", expect: 12 }, { call: "makeMultiplier(0)(5)", expect: 0 }, { call: "makeMultiplier(-2)(2)", expect: -4 }], solution: "function makeMultiplier(x) {\n  return (y) => x * y;\n}", explain: "The returned function closes over x, so every call remembers its multiplier." }] } },
            ...
            },
            {
              ...
              steps: [
                ...
                { type: "code", prompt: "Write maxOf: return the largest number in a non-empty list, using reduce (no Math.max).", starter: "function maxOf(list) {\n  \n}", tests: [{"call": "maxOf([3,9,4])", "expect": 9}, {"call": "maxOf([-5,-2])", "expect": -2}, {"call": "maxOf([7])", "expect": 7}], solution: "function maxOf(list) {\n  return list.reduce((best, n) => (n > best ? n : best), list[0]);\n}", explain: "Carry the best-so-far. Starting from list[0] keeps negative-only lists honest.", gen: { variants: [{ prompt: "Write productOf: return the product of a non-empty list of numbers, using reduce.", starter: "function productOf(nums) {\n  \n}", tests: [{ call: "productOf([2, 3, 4])", expect: 24 }, { call: "productOf([7])", expect: 7 }, { call: "productOf([2, -3])", expect: -6 }], solution: "function productOf(nums) {\n  return nums.reduce((p, n) => p * n);\n}", explain: "With no start value, reduce seeds itself with the first item and folds in the rest." }, { prompt: "Write minOf: return the smallest number in a non-empty list, using reduce (no Math.min).", starter: "function minOf(nums) {\n  \n}", tests: [{ call: "minOf([4, 2, 9])", expect: 2 }, { call: "minOf([7])", expect: 7 }, { call: "minOf([-1, 5])", expect: -1 }], solution: "function minOf(nums) {\n  return nums.reduce((m, n) => (n < m ? n : m));\n}", explain: "With no start value, reduce seeds itself with the first item and folds in the rest." }] } },
            ...
            },
            {
              ...
              steps: [
                ...
                { type: "code", prompt: "Write withDone: return a copy of task with done set to true, WITHOUT modifying the input.", starter: "function withDone(task) {\n  \n}", tests: [{"call": "withDone({title:\"a\", done:false})", "expect": {"title": "a", "done": true}}, {"call": "(function(){ const t = {title:\"x\", done:false}; withDone(t); return t.done; })()", "expect": false}], solution: "function withDone(task) {\n  return { ...task, done: true };\n}", explain: "The second test proves the original survived. That property is what makes state changes easy to reason about.", gen: { variants: [{ prompt: "Write withStarred: return a copy of item with starred set to true, WITHOUT modifying the input.", starter: "function withStarred(item) {\n  \n}", tests: [{ call: "withStarred({id:1, starred:false})", expect: { id: 1, starred: true } }, { call: "(function(){ const t = {id:2, starred:false}; withStarred(t); return t.starred; })()", expect: false }], solution: "function withStarred(item) {\n  return { ...item, starred: true };\n}", explain: "Spread copies the old drawers into a fresh object, then the override lands on the copy only." }, { prompt: "Write withArchived: return a copy of note with archived set to true, WITHOUT modifying the input.", starter: "function withArchived(note) {\n  \n}", tests: [{ call: "withArchived({text:\"a\", archived:false})", expect: { text: "a", archived: true } }, { call: "(function(){ const t = {text:\"x\", archived:false}; withArchived(t); return t.archived; })()", expect: false }], solution: "function withArchived(note) {\n  return { ...note, archived: true };\n}", explain: "Spread copies the old drawers into a fresh object, then the override lands on the copy only." }] } },
